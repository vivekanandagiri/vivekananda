...
const TechStack = () => {
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4 sm:gap-6 px-2 sm:px-4">
        {techs.map((tech, idx) => (
          <div
            key={idx}
            className="flex flex-col items-center bg-slate-800 rounded-lg p-3 sm:p-4 border border-cyan-100 shadow hover:shadow-lg transition hover:bg-violet-700 hover:border-cyan-400"
          >
            {tech.icon}
            <span className="mt-2 text-cyan-200 font-semibold text-xs sm:text-sm md:text-base text-center">{tech.name}</span>
          </div>
        ))}
      </div>
    </div>
